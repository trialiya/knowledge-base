...
const chatApi = {
  ...
  /**
   * Откатить файловые правки последнего ответа: названные файлы (пустой список — все, что ещё не
   * откачены) возвращаются к состоянию до него, а в истории остаётся ряд об этом. Репозиторий не
   * передаётся: его называет история самого чата — селектор проекта могли переключить уже после
   * ответа. Ответ — { id, createdAt, event: { project, paths } }, из которого лента рисует плашку,
   * а модель узнаёт об откате на следующем ходу.
   *
  ...
  revertFiles: async (id, paths = []) => {
    const res = await requestRaw(`/api/chats/${enc(id)}/revert-files`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
