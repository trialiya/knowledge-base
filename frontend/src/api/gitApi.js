...
const gitApi = {
  ...
  /**
   * Адрес сырых байт файла — не запрос, а ссылка: её подставляют в `src`
   * картинки, и грузит её сам браузер. Поэтому здесь нет ни `signal`, ни
   * разбора ответа — только адрес.
   *
   * Отдаются так не любые файлы, а только те, чей тип бэкенд знает по
   * расширению (картинки, SVG); на остальные он отвечает 415 — что показывать
   * как рисунок, решает общий с ним список (utils/filePreview.js).
   */
  rawUrl: (path, { rev, project } = {}) => {
    const params = new URLSearchParams({ path });
    if (rev) params.set('rev', rev);
    if (project) params.set('project', project);
    return `/api/git/files/raw?${params.toString()}`;
  },
