 ...
 * ──────────────────────────────────────────────────────────────────────────
 *
 * Схему URL целиком описывает navUrl.js, а пишет в window.history один лишь
 * navStore.js. Здесь живёт ровно одна её часть: как из ресурса собрать
...
/**
 * Полный адрес чата: путь плюс запрос find-бара и сообщение, на котором сюда
 * пришли из поиска.
 *
 * Оба — состояние экрана, а не часть идентичности чата, но живут именно в
 * адресе: ссылкой на найденное сообщение делятся, и Ctrl+клик с карточки
 * результата обязан открыть чат на том же сообщении и с той же подсветкой, что
 * была в выдаче. Сообщение без запроса не пишется — вести к нему некому.
 */
export function chatUrl(chatId, { find, msg } = {}) {
  const p = new URLSearchParams();
  if (find) p.set('find', find);
  if (find && msg) p.set('msg', String(msg));
  const qs = p.toString();
