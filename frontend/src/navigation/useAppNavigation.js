...
export default function useAppNavigation() {
  ...
  /**
   * Что подсвечено в открытом документе — по тем же правилам. Раздел при этом
   * снимается: он относился к запросу, с которым пришли из поиска, а набранный
   * в баре запрос начинает с первого совпадения, как везде.
   */
  const setDocFind = useCallback(
    (find) => {
      const next = find || '';
      replaceNav((prev) =>
        prev.docFind === next && !prev.docSection ? prev : { ...prev, docFind: next, docSection: '' },
      );
    },
    [replaceNav],
  );
