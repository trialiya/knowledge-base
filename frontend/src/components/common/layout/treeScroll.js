 ...
 *
 * Отсюда два входа, и делит их намерение зовущего, а не устройство панели.
 * `revealRow` двигает обе оси — его зовёт дерево файлов, доводя себя до
 * выбранного узла. `revealVertically` горизонталь не трогает: дереву базы
 * знаний её двигать некуда (своей прокрутки по ней у панели нет, длинное имя
 * обрезается многоточием), а клавиатурная навигация ходит и по дереву файлов
 * тоже — и там обязана оставить горизонтальную прокрутку как есть, иначе шаг
 * стрелкой сбрасывал бы её в ноль на каждой строке. Разобрано в
 * `common/search/useListNavigation.js`; не «чините» его в обе оси.
 *
