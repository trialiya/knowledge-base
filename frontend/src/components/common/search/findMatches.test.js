import { buildMatcher, collectMatchRanges } from './findMatches';
...
/** В ленте чата ищут по тексту сообщений, а не по времени и карточкам вызовов. */
test('within сужает область до нужных блоков', () => {
  const root = document.createElement('div');
  root.innerHTML = '<div class="message">needle в сообщении</div><time>needle в отметке</time>';
  document.body.appendChild(root);

  const found = collectMatchRanges(root, buildMatcher('needle', false), { within: '.message' });
  expect(found.map((r) => r.startContainer.nodeValue)).toEqual(['needle в сообщении']);
});
