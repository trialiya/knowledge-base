...
export default function useInChatSearch({
  ...
  if (prevChatId !== activeChatId) {
    ...
    // приносит чат и запрос одним адресом, и безусловный сброс стирал бы ровно
    // тот запрос, ради которого переходили, — бар не открывался бы вовсе.
    // Обычное переключение чата мышью адреса с запросом не несёт: там это
    // по-прежнему закрытый пустой бар.
    setQuery(find || '');
    setOpen(!!find);
  ...
  // нажали «Назад». Чат и запрос приходят из одного адреса в одном рендере;
  // сменились оба — блок выше уже взял новый запрос, и здесь он лишь
  // подтверждается.
