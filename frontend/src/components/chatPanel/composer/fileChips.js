...
export function parseToken(token) {
  ...
  // Грамматика при этом остаётся неоднозначной: путь, который сам кончается на `#N-M`, читается
  // как путь с диапазоном. Так и задумано — диапазоны в чипах обычны, а такое имя (без
  // расширения, из цифр через дефис) в репозитории не встречается.
