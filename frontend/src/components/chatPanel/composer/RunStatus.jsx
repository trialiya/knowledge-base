...
/**
 * Строка над полем ввода на время занятости чата: сколько она уже идёт и на сколько токенов вырос
 * input. Поле ввода при генерации не блокируется — эта строка и говорит пишущему, что чат занят и
 * чем именно.
 *
 * Секундный тик живёт здесь, а не в композере: перерисовывается одна эта строка. Прирост — из
 * живых снимков RUN_USAGE (см. runInputGrowth); провайдер без usage в стриме числа не даст, а у
 * сжатия его не бывает вовсе — и тогда остаётся один таймер. Якоря может не быть у прогона, чей
 * RUN_STARTED ещё не доехал, — тогда наоборот, до якоря показывается только прирост.
 *
 * @param startedAt момент старта по часам ЭТОЙ вкладки (chat.runStartedAt), null — не известен
 * @param inputGrowth прирост input за прогон в токенах, null — не измерен или неприменим
 */
