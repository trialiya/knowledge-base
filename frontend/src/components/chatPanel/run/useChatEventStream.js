  ...
  useEffect(() => {
    ...
    closeStream = openChatEventStream(chatId, {
      ...
      onEvent: (ev) => {
        ...
        // Откат правок ответа — то же самое: файлы вернулись к прежнему виду, и вкладка,
        // которая его не запускала, обязана перечитать дерево и открытый файл наравне с той,
        // которая запускала (своё же событие она получает обратно).
        if (ev.type === CHAT_EVENT.FILE_REVERT) {
          fireRepoChanged();
        }
        // Прогон по команде `/script` мог писать в дерево (права у него проектные, а не
        // стендовые): какие файлы — знает только сам прогон, поэтому сигнал тот же общий.
        if (ev.type === CHAT_EVENT.SCRIPT_RUN) {
          fireRepoChanged();
        }
        ...
        if (TERMINAL_EVENTS.has(ev.type)) {
          // Прогон завершён: хаб очистит свой лог, и события следующего прогона курсор от
          // этого не покрывает — он остался на событиях, которых в логе уже нет. Сбрасываем
          // его, чтобы переподписка сделала полный реплей нового прогона. Сжатие
