...
const toRunState = (active) => {
  ...
  return {
    ...
    // elapsedMs — сколько операция уже идёт по часам сервера: длительность, а не момент
    // старта, поэтому перекос часов клиента и сервера якорь таймера не портит. Есть и у
    // генерации, и у сжатия: ждать и там и там приходится одинаково.
