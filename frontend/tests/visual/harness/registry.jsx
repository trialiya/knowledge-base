 ...
 *
 * И не всё, чему хватает. Запись стоит двух эталонов (светлый и тёмный) и
 * своей доли прогона, а окупается только тем, что её diff поймает то, чего
 * не поймает diff ни одной другой. Заводится, когда снимок показывает
 * компонент, которого в реестре ещё нет; состояние, в которое только эта
 * фикстура его и приводит (незавершённый merge, упавший ответ, «скопировано»);
 * или сочетание, в котором уже снятые части могут столкнуться — длинное имя
 * против крошек, список вверх над узким полем. НЕ заводится под фикстуру,
 * которая целиком входит в другую (список без одной строки), под тот же
 * компонент с другой подписью, числом или флагом, вид которых уже есть на
 * соседнем снимке, и под сборку из общих компонентов — кнопок, бейджей, строк
 * InfoList — в уже снятой рамке: каждая из частей снята, а их встреча ничего
 * нового не рисует. Проверка перед добавлением: «что этот снимок покажет
 * сломанным, чего не покажет никакой другой?» Нет ответа — кейс в cases.yaml
 * описывается, а снимка не получает. Сомнение решается в пользу снимка, но с
 * комментарием, ради чего он: без него следующий разбор реестра его же и
 * снимет.
 *
 * `id` совпадает с ссылкой на фикстуру в `cases.yaml` — `<модуль>#<экспорт>`,
 * — чтобы кейс и снимок находились друг по другу без второго имени. Совпадение
 * сторожит `../registry.test.js`: заведённая здесь запись, которую ни один кейс
 * не описывает, и ссылка из кейса в никуда одинаково роняют фронтовые тесты. Одну и ту
 ...
 * указатель уводит — иначе снимок «доказывает» то, чего в правилах нет.
 *
...
const LIGHT = [
  ...
  ...[
    ['', undefined],
    ['@open', [{ click: '.header-menu__trigger' }, { unhover: true }]],
  ].map(([variant, steps]) => ({
  ...
  // Список различий: смешанный список плюс удалённый с диска узел — единственный
  // статус, который может что-то стереть; в живом прогоне он не наблюдался,
  // поэтому и рисуется только здесь. `showUnchanged` выключен, как после
  // сравнения, — совпавшие записи прячутся, и в шапке остаётся счётчик скрытых.
  // Отдельного снимка одного смешанного списка нет: он целиком входит в этот.
  {
