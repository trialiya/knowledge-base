 ...
 * хеш, плюс две `block`-строки коммита — тема и тело сообщения. На нём и видно,
 * как встают кнопки копирования (в один ряд со значением и отдельно в
 * многострочной block-строке) и как тело сохраняет свои переносы (`pre`).
 */
export const fileRows = [
  ...
  },
  {
    label: 'Message body',
    value:
      'Позиция фразы больше не пересчитывается на клиенте: сервер отдаёт её сам.\n\n' +
      'Осталось перенести на тот же запрос историю — там та же арифметика,\n' +
      'только по другому ключу.',
    block: true,
    pre: true,
  },
];
...
/**
 * Тот же набор из чата, но проект чата исчез из конфигурации: подписи у него
