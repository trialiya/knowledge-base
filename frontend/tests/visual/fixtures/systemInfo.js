 ...
 *
 * Группа `build` заполнена целиком — ради снимка, чтобы в нём были видны все строки
 * секции. Такой сборки сегодня не существует: коммит есть у той, что видит `.git`
 * (а она не пишет времени), время — у образа из docker/Dockerfile (`-Pkb.build.time`,
 * но `.git` в стадию не копируется). Так что вживую часть строк обычно «неизвестно»,
 * а у запуска прямо из классов (bootRun) — все. Поведение на null проверяет
 * SystemInfoControllerTest, отдельного снимка под него нет.
 */
