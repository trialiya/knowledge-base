 ...
 *
 * Группа `build` заполнена целиком — так выглядит релизный артефакт: коммит кладёт
 * плагин git-properties, время сборки — `-Pkb.build.time` (его передаёт только
 * docker/Dockerfile, обычная локальная сборка `builtAt` не пишет). У запуска прямо
 * из классов (bootRun) там всюду null, и панель показывает «неизвестно»; поведение
 * на null проверяет SystemInfoControllerTest, отдельного снимка под него нет.
 */
