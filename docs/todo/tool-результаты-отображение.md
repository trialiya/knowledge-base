...
`outline` из первоначального наброска отдельным видом не понадобился: оглавление документа и
обзор файла — это та же иерархия, просто выраженная уровнем заголовка и диапазоном строк
вместо ссылки на родителя. Один рендерер, четыре сборщика.
...
Порядок — это ещё и способ **уточнять уже работающий отбор, не переписывая его**. Так и вышло
на деле: `getTreeSkeleton` и `getFileTree` сперва попали в `recordList` и показывались плоским
списком — честно, но без иерархии; появившийся `tree` встал выше и забрал их себе, а разбор в
`recordList` остался рабочим и ничего не потерял.

Виды не спорят за форму, а делят её одним правилом на двоих: `recordList` и `tree` не берут
массив, элементы которого несут длинный текст (это `content`), а `scalar` берёт ровно
дополнение к «длинному тексту» — короткую однострочную строку. Оба предиката
(`carriesContentText`, `isContentText`) живут в `contentResult.js` и импортируются, а не
переписываются: два порога разъехались бы на первой же правке.

Отдельный случай — `grepContent`. Его `text` формально длинный и многострочный, то есть по
предикату это «содержимое», но нумерация уже внутри самого текста (`:85:` — совпадение,
`-84-` — контекст), и текстовый вид нарисовал бы поверх неё второй столбец номеров, считающий
от единицы. Поэтому `grepMatches` стоит выше `content` и разбирает разметку в настоящие номера
строк; в `content` от той поры остался отбой записей с `matchLine` — как защита на случай
одиночного объекта такой формы.
...
- **Тёмная тема.** Цвет везде берётся ролью (`styles/theme-light.css`), литералов
  в CSS панели нет — новые блоки пишутся так же, иначе их завалит
