...
### 4.1 Три режима на секцию результата

Сегментированный переключатель в заголовке секции, рядом с кнопкой копирования:

```
Результат                            [ Обзор │ JSON │ Сырой ]   ⧉
```

- **Обзор** — типизированный рендер (раздел 4.3). Режим по умолчанию, если для формы
  нашёлся рендерер.
- **JSON** — сворачиваемое дерево вместо `pre`: объекты и массивы схлопываются
  (`▸ files [12]`), длинные строки — как в 4.2. Режим по умолчанию для незнакомых форм.
- **Сырой** — ровно `resultText`, байт в байт то, что видела модель. Это единственный
  режим, который нельзя убирать: при разборе «почему модель так решила» важно видеть
  вход модели, а не наш красивый пересказ.

Выбранный режим держится в состоянии модалки (не в URL — модалка не адресуемая) и
сбрасывается на дефолт при смене `callId`.

Ту же тройку получает секция **Аргументы** — по тем же причинам (см. `editFile`,
`createDocument`), только «Обзор» для аргументов — это плоский список
...
```
│ Результат  · 12 записей         [ Обзор │ JSON │ Сырой ]  ⧉  │
...
1. **Правило длинной строки (4.2) + сворачиваемое JSON-дерево.** Один вьювер, ноль
   реестров. Чинит `getFileContent`, `getCommitDiff`, `editFile`, `createDocument`
   и MCP разом. Плюс: то же дерево — секции аргументов.
2. **Три режима (4.1) и шапка фактов (4.4).** «Сырой» появляется здесь, до всякой
   типизации, — чтобы «Обзор» никогда не был единственным источником правды.
3. **Реестр форм (4.3) с четырьмя рендерерами**, закрывающими 21 из 36 инструментов:
   `diffList` (вынести из `FileDiffModal`), `fileContent`, `recordList`, `scalar`.
4. **Специализированные:** `outline`, `tree`, `grepMatches`, `markdownText`,
   `docMutation` (переиспользуя `DocChangeBlock`), `scriptRun`.
5. **Ограничение объёма (4.6)** — когда появятся вьюверы, которым есть что ограничивать.

Этап 1 — большая часть выигрыша при наименьшем объёме; этапы 3–4 добавляют удобство
там, где форма известна.
