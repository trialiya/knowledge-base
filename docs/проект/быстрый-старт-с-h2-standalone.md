...
В отличие от остального приложения, у консоли нет отдельного запроса Basic-логина —
её страница открывается сразу, без пароля из `kb.security`. Второй пароль поверх
JDBC-логина (поля выше) не добавлял бы защиты, только лишний клик, поэтому вход в
саму БД остаётся единственным барьером. Профиль `h2` рассчитан на локальную
разработку и демо-стенды — не публикуйте порт `8080` в интернет без дополнительной
защиты (например, обратного прокси с собственной аутентификацией).
...
Готовый артефакт — `backend/build/libs/backend-1.0.0-RC1.jar`, данные H2 — в каталоге `local-db/` рядом с процессом. Требуется JDK/JRE 25.
