...
Консоль закрыта той же Basic-аутентификацией, что и остальное приложение: браузер
сначала спросит логин/пароль из `kb.security` (по умолчанию `admin` / `admin`), и
только потом покажет форму подключения к БД с полями выше.
