...
поэтому список выдаётся в системном промпте (`ProjectPromptService`): активный
проект, затем те, на которых чат уже работал (с пометкой «выбирались раньше» —
пути и содержимое выше по истории принадлежат им), затем остальные настроенные.
Проекты, чей репозиторий не открылся, не перечисляются вовсе. История берётся из
маркеров смены проекта (`ChatHistoryService#earlierProjects` поверх
`chat_message.meta`) — сам чат хранит только текущий проект. Отбирает их
`ChatMessageRepository#findProjectSwitches`: точно по `type = 'USER'` (маркер
ставится только на вопрос) и приблизительно по подстроке в `meta` — часть рядов
хранит поле выписанным в `null`, так что окончательно решает разбор меты. Выдуманный id падает
на `ProjectCatalog#require` с перечислением настроенных, то есть громко.
