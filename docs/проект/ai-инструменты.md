...
- **Ограничения:**
  ...
  - **Чтения заранее не требуется** — безопасность даёт сам контракт точного совпадения: привести уникальный фрагмент может только тот, кто видел текущее содержимое, а несовпадение возвращает ошибку с именем инструмента для перечитывания. Отдельное правило «read-before-edit» осталось только у скриптовой песочницы (`kb.edit` / `kb.write`, см. `ScriptSession.requireRead`): скрипт пишет ещё и файлы целиком, где такого контракта нет
  - По умолчанию отключён (read-only)
...
Три read-only инструмента — `getFileContent`, `grepContent` и `runScript` —
принимают сверх этого необязательный аргумент `project`, который перекрывает
проект прогона на один вызов. Он существует ради единственного случая, который
контекст обслужить не может: кросс-проектного вопроса, заданного намеренно
(«как это сделано в A, а как в B»). Пропущенный или пустой `project` означает
проект чата, так что обычный вызов остаётся прежним.

Цена такого аргумента — та самая молчаливая подмена, поэтому ответ каждого из
трёх **называет проект, который на самом деле отвечал**: поле `project` в
`GitFileContent`, в каждом `GitGrepMatch` и в `ScriptResult`. Промпт (`sys.md`)
