...
- **Параметры:** `filePath` (String), `content` (String), `overwrite` (boolean, default false)
...
- **Ограничения:** 
  ...
  - Если файл существует и `overwrite=false` — возвращается ошибка (используйте `editFile` или установите `overwrite=true`)

### `editFile`
Редактирование существующего файла в репозитории (замена текста, точное совпадение).
- **Параметры:** `filePath` (String), `oldString` (String), `newString` (String)
