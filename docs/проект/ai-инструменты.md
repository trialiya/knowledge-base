...
инструмента увидит `null` и ответит. Для **примитива** рефлексия бросает
`IllegalArgumentException("argument type mismatch")` из самого `invoke`, а
...
`ToolArgumentGapsTest` — он гоняет каждый `@Tool` через настоящий
`MethodToolCallback` с телом `{}`. Примитив в сигнатуре инструмента ломает этот
тест.
