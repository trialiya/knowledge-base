...
- **Возвращает:** список `GitFileNode`; у каждого узла поле `project` — id репозитория, которому он принадлежит
...
- **Возвращает:** `GitFileContent` с полями: `project` (id репозитория, из которого прочитан файл), `path`, `content` (текст или null для бинарных), `binary`, `sizeBytes`, `language` (определяется по расширению), `lineCount` (всего строк), `truncated` (true если вернулась только часть), `fromLine`/`toLine` (фактический диапазон)
...
- **Возвращает:** `OutlineResult` — обёртка над `GitFileOutline` (поля `project`, `path`, `language`, `lineCount`, `parser`) со списком `GitSymbol` (имя, `startLine`, `endLine`)
...
- **Возвращает:** список найденных файлов (`GitFileNode`, каждый с полем `project`)
...
- **Возвращает:** список `GitCommit`; поле `project` называет репозиторий, из истории которого взят коммит
...
- **Возвращает:** список изменённых файлов (`GitDiffEntry`, каждый с полем `project`) + diff (опционально)
...
**называет проект, который на самом деле отвечал**: поле `project` есть в
`GitFileContent`, `GitGrepMatch`, `GitFileNode`, `GitCommit`, `GitDiffEntry`,
`GitFileOutline`, `SearchAgentResult` и `ScriptResult`. Промпт (`sys.md`)
требует брать id для ссылки `/files?path=…&project=…` из ответа, а не из
активного проекта — иначе ссылка на файл соседнего репозитория открыла бы файл
с тем же путём в своём.

Эхо — не только для модели: `ToolInvocationCollector#hasSeenFile` сверяет по нему
проект, чтобы файл, прочитанный в одном репозитории, не засчитался за «видели» для
записи одноимённого пути в другом. Поэтому инструмент, получающий аргумент
`project`, обязан в том же изменении получить и эхо, и строку в
`PROJECT_AWARE_TOOLS`.
