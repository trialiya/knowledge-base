...
Read-only эндпоинты для автодополнения `/file` в композере чата и для файлового браузера (раздел «Файлы»). Делегируют в `GitService`, который обеспечивает доступ только к tracked-файлам, защиту от path traversal и ограничения по бинарным/большим файлам.
