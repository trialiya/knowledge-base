...
Пока в конфигурации допускается ровно одна запись `kb.projects` (см.
...
дефолтный; проект, исчезнувший из конфигурации, не ломает чат, а молча уступает дефолтному.
