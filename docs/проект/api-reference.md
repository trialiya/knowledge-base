...
Сжать контекст чата — бэкенд команды `/compact` в композере. Весь живой контекст (несжатые
сообщения плюс уже накопленные сводки) уходит модели **одним запросом, теми же сообщениями,
что и в обычном прогоне**: протокольные `tool_calls` с полными аргументами и TOOL-ответы с
полными результатами внутри. Результат заменяет всё окно одной строкой-сводкой — живого хвоста,
в отличие от фоновой суммаризации, не остаётся.

Сама команда в историю не пишется: на её место в запрос к модели встаёт инструкция сжатия
(системный промпт — `prompt/compactor.md`). Модель — та, на которой работает чат.
...
**Body:** `CompactRequest` (JSON, опционально) — хвост команды `/compact <текст>`.
```json
{ "instructions": "подробнее про миграции" }
```

`instructions` уходит фокусом сжатия: названное получает больше деталей, но остальные разделы
сводки всё равно заполняются полностью.
...
- `409 Conflict` — чат занят: идёт генерация или другое сжатие
