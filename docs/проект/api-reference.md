...
Пока в конфигурации допускается ровно одна **включённая** запись `kb.projects` (см.
[конфигурацию](конфигурация.md)), поэтому список из одного элемента — это нормальное состояние, а
не признак ошибки.
...
Каждый эндпоинт принимает необязательный `project` — id проекта из `GET /api/chats/projects`. Без него отдаётся дефолтный проект (первый в `kb.projects`), с неизвестным — `400`: открыть «тот же путь, но в другом репозитории» молча хуже, чем не открыть ничего.
