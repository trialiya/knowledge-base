...
**Response:** `List<GitDiffEntry>` — `{ status, path, oldPath, additions, deletions, patch }`. Статусы: `A`, `M`, `D`, `R`, `C`, `U`. У файла без незакоммиченных изменений ответ — пустой список, а не `404`.
