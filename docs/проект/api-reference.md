...
### PUT `/api/chats/{conversationId}/project`
Выбрать проект для чата. Пустое тело — сброс к дефолтному.

**Body:** `String` — id проекта (raw text) или пусто

**Response:** `200 OK`

**Ошибки:** `403` — чужой чат, `400` — неизвестный проект
...
дефолтному (см. про `Chat.project` выше).
