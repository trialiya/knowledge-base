...
- Помни, к какому состоянию относится вопрос. `getCommitDiff` показывает конкретный коммит и остаётся источником истины для вопросов про этот коммит; `getFileContent` / `getFileOutline` читают только текущее состояние (worktree), а не историческую ревизию — не «поправляй» ими дифф старого коммита. Если вопрос про **текущее** состояние кода, не делай окончательный вывод только по диффу: файл мог измениться позже — проверь текущий файл через `getFileContent` / `getFileOutline` или поиск. Если файл в диффе позже удалён/переименован, чтение текущего пути ничего не подтвердит — так и скажи.
