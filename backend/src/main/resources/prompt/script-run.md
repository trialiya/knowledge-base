...
**Повторный вызов с теми же аргументами ничего не стоит.** `kb.files`, `kb.read`, `kb.outline`,
`kb.grep`, `kb.searchDocs` — все пятеро отдают один и тот же ответ на один и тот же вызов
(с точностью до аргументов), и второй такой вызов в этом прогоне возвращается из кэша, а не
считается заново: не тратит `kb.*`-вызов и не добавляет байтов к прочитанному. Так что пиши код,
как выходит естественно: не нужно ни выносить чтение из цикла заранее, ни специально избегать
повторного чтения одного файла — то и другое просто не стоит ничего.
