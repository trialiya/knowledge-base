...
считается заново: не тратит `kb.*`-вызов и не добавляет байтов к прочитанному. Это не про экономию —
можно писать код так, как естественно выходит, не выдумывая, куда заранее вынести чтение из цикла.
