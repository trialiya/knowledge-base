...
5. **Порядок вызовов в цикле можно не продумывать заранее.** Повторный `kb.read`/`kb.grep`/… с теми
   же аргументами ничего не стоит (см. справочник), так что для задачи «для каждого файла — для
   каждого имени» пиши как удобнее: хоть `kb.read` прямо во внутреннем цикле, хоть один проход
   заранее в объект — оба варианта одинаково дёшевы, выбирай тот, что читается яснее.
...
Проверить список имён против множества файлов — `kb.read` внутри внутреннего цикла, без отдельного
прохода «сначала всё прочитать»: одинаковый вызов на одном и том же файле отдаётся из кэша прогона
(см. справочник kb), а не считается заново, так что расточительности тут нет:
```js
var usage = {};
