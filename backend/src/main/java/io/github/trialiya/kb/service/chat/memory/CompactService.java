 ...
 * посреди сжатия нельзя — {@code sys.md} учит модель начинать ответ с чтения, и без этого запрета
 * сжатие ходило бы кругами вместо документа (а в развёртывании с правом записи могло бы и тронуть
 * репозиторий). Второй рубеж — сам {@code compactor.md}, который явно снимает роль из системного
 * промпта.
 *
...
public class CompactService {
    ...
    public CompactPayload compact(
        ...
        // sys.md как раз учит начинать ответ с вызова — то есть вероятная форма такого ответа не
        // пустая, а «сейчас посмотрю» плюс сам вызов. Пропусти мы её по непустому тексту, этой
