...
public final class ToolInvocationCollector {
     ...
     * этом ответе — правило «read before edit» скриптовой песочницы ({@code kb.edit} и {@code
     * kb.write}, см. {@code ScriptSession#requireRead}). Живёт здесь, а не внутри песочницы, потому
     * что спрашивает про историю всего ответа: файл, прочитанный обычным инструментом до скрипта,
     * засчитывается наравне с чтением внутри него.
     *
     * <p>Инструмент {@code editFile} этого правила не спрашивает: там гарантию даёт точное
     * совпадение {@code oldString} (см. {@code GitEditFunction}). В {@link #PATH_ARG_READ_TOOLS} он
     * остаётся как свидетельство — правка файла означает, что модель его содержимое видела.
     *
     ...
     *
     * <p>Но не проект: чтение файла в одном репозитории не должно засчитываться за правку
     ...
     * @param project id проекта, куда пойдёт правка (канонический — тот же, что несёт эхо ответа)
     */
