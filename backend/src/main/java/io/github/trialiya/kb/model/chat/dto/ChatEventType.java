...
public enum ChatEventType {
    ...
    TOOL_CALLS,
    /**
     * Токены, потраченные прогоном к этому моменту ({@code payload}: {@link TokenUsage}) —
     * нарастающий итог, а не добавка: событие приходит по нескольку раз за прогон, и каждое
     * следующее заменяет предыдущее целиком. Провайдер без поддержки usage в стриме не присылает
     * его ни разу, и это отличается от нуля — считать нечего, а не насчитали ноль.
     */
    RUN_USAGE,
