...
public class ChatRunService {
    ...
    private void persistPartial(
        ...
        try {
            ...
            // каждый следующий запрос этого чата). Область прогона передаём с собой: успевшие
            // вызовы брошенного батча потеряли ответ вместе с ним, а их результат помнит только
            // она — здесь он ещё есть, у чужих путей ремонта уже нет.
            chatHistory.repairDanglingToolCalls(conversationId, scope);
            if (!partial.isBlank()) {
