...
/**
 * Токены, потраченные прогоном (см. {@code TokenUsageAdvisor}). Одно число на прогон, а не на вызов
 * модели: пользователь спрашивает «сколько стоил ответ», а ответ с инструментами — это несколько
 * оплаченных обращений к модели подряд.
 *
 ...
 *       отдельно, и prompt в ней считается заново от начала диалога.
 * </ul>
 *
