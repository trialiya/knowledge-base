 ...
 * трёх одна и та же граница жизни, а каждый отдельный реестр требовал бы своей дисциплины очистки,
...
public final class RunScope {
    ...
    /**
     * Запрошена ли остановка — это же читают инструменты прогона через {@code RunCancellation}.
     * Наружу уходит ответ, а не сам флаг: ставит его только {@link #cancel}, вместе с гашением
     * подписки, а флаг, поставленный в обход, остановил бы инструменты, не остановив генерацию.
     */
    ...
    public RunTokenUsage usage() {
        return usageWith(TokenUsage.EMPTY);
    }
