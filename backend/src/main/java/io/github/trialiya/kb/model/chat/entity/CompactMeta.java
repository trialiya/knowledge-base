...
public record CompactMeta(
     ...
     * #AUTO_COMPACT} заменяют сводкой весь живой контекст, а {@link #SUMMARIZE} и {@link
     * #COMPACT_KEEP_LAST} — только его часть, и живой хвост остаётся. Отсюда разное чтение замера
     * на плашке (см. {@code tokenUsage.js} на фронте): после полного сжатия контекст оценивается по
     * сводке, после частичного — не оценивается вовсе, потому что размер оставшегося хвоста
    ...
    public enum Kind {
        ...
        /**
         * Команда пользователя {@code /compact-1}: то же сжатие, но последний ход разговора
         * оставлен живым (см. {@code CompactWindow}). Читается как частичное — под плашкой остался
         * хвост, размер которого ниоткуда не известен.
         */
        COMPACT_KEEP_LAST,
