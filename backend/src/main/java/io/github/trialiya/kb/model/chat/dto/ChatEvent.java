package io.github.trialiya.kb.model.chat.dto;

import org.jspecify.annotations.Nullable;

/**
 * Конверт одного события в канале чата.
 *
 * @param seq номер события: даёт порядок и точку дозагрузки при переподключении. Монотонный, но не
 *     плотный — номера раздаёт счётчик, общий на весь процесс ({@code ConversationHub#SEQ}), так
 *     что в одном чате они идут с пропусками, а через рестарт прыгают по часам. Считать разрывы
 *     потерей событий нельзя
 * @param type тип события
 * @param runId к какому прогону генерации относится (null для чисто служебных событий)
 * @param clientMsgId идентификатор клиента-инициатора — чтобы вкладка-отправитель не задвоила свой
 *     оптимистично показанный пузырь
 * @param payload полезная нагрузка, зависит от {@link #type}
 */
public record ChatEvent(
        long seq,
        ChatEventType type,
        @Nullable String runId,
        @Nullable String clientMsgId,
        @Nullable Object payload) {}
