...
public class GitFunction {
     ...
     * перечисляет пути столбиком. Перечисление столбиком делится только по строкам: запятая в имени
     * файла — законный символ (см. {@code RepoPaths}), и там, где разделитель виден по-другому,
     * ломать об неё путь не за чем. Пустой список означает «всё дерево» — как и пропущенный
     * аргумент.
     */
    private static List<String> pathList(@Nullable String paths) {
        ...
        return Arrays.stream(paths.split(paths.indexOf('\n') < 0 ? "," : "\n"))
                .map(String::strip)
