...
public class GitFunction {
     ...
     * перечисляет пути столбиком — а чаще всего смешивает и то и другое, и разбор обязан понимать
     * такой список целиком, а не половину.
     *
     * <p>Цена этого — единственная: путь с запятой в имени (символ законный, см. {@code RepoPaths})
     * как фильтр не назвать, он разломится надвое и не совпадёт ни с чем. Вызов без фильтра такой
     * файл по-прежнему показывает.
     *
     * <p>Пустой список означает «всё дерево» — как и пропущенный аргумент.
     */
