...
public class ConversationHub {
     ...
     * #ownCursor}).
     *
     * <p>Через рестарт монотонность держит засев временем: часы за жизнь процесса уходят вперёд
     * быстрее, чем он успевает опубликовать миллион событий в секунду, так что счётчик нового
     * процесса стартует выше всего, что раздал прежний. Номера общие на все чаты — у одного хаба
     * они идут с пропусками, но клиенту {@code seq} непрозрачен: он только сравнивает его с
     * курсором.
     */
     ...
     * граница проверяется тоже: номера больше выданных не бывает ни у одного курсора, а раз так,
     * это не курсор.
     *
