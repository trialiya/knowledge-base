...
public class SummaryWriter {
     ...
     *     живого хвоста после него не остаётся, поэтому сводке нечего обгонять. Время видимой
     *     плашки задаётся отдельно (см. {@link CompactStats#noticeAt}): у сводки это время решает,
     *     где она встанет в промпте, а у плашки — где её увидит человек, и совпадают они не всегда
     ...
     *
     * <p>Позиция плашки — сразу за сводкой, и занятой она быть вправе: сжатие, оставившее за собой
     * живой хвост, даёт тот же номер его первому ряду. Порядок от этого не страдает — история и в
     * ленте, и в промпте читается по времени, а номер плашке нужен только затем, чтобы следующий
     * раунд накрыл её своей разметкой (модели она не едет и так).
     *
     ...
     * @param noticeAt время плашки в ленте. Отдельно от времени сводки, потому что отвечают они на
     *     разные вопросы: время сводки решает, где она встанет в промпте (перед живым хвостом), а
     *     время плашки — где её увидит человек. У {@code /compact-1} они расходятся: сводка
     *     подписана последним сжатым рядом, чтобы уехать модели ПЕРЕД сбережённым ходом, а плашка —
     *     концом раунда, чтобы встать в ленте под командой, которая её вызвала
     */
