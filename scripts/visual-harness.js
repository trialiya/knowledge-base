 ...
 * рисует иначе, чем в песочнице (другой набор шрифтов — и расходятся три кейса
 * из четырёх), поэтому эталоны принадлежат окружению, а не репозиторию вообще.
 *
 ...
 *
 * Пиксель считается разошедшимся, когда канал отличается БОЛЬШЕ ЧЕМ НА ЕДИНИЦУ.
 * Единица — это измеренный шум, а не запас «на всякий случай»: круглая кнопка
 * и скруглённая карточка дают по кромке дугу в полтора-два десятка пикселей,
 * которые от прогона к прогону гуляют ровно на 1/255 (замерено на кнопке меню
 * в шапке: 80 каналов, максимум расхождения — 1). Дуга связная, и порогом по
 * размеру области её не отсечь — она крупнее его. Настоящая правка оттенка
 * столько не весит ни разу: соседние ступени палитры расходятся на десятки.
 *
 * Второй шум — по площади: скруглённые
 ...
 *
 * Отсюда вторая мера — не доля разошедшихся пикселей, а размер связной области,
 * в которую они складываются: `minCluster` отсекает эти крапины, а настоящая
...
async function compare(page, shotFile, baseFile, minCluster) {
  ...
  return page.evaluate(async ([shotUrl, baseUrl, floor]) => {
    ...
    // Дрожание растеризации — ровно на единицу канала; всё, что больше, это
    // уже чья-то правка (см. шапку).
    const NOISE = 1;
    const changed = new Uint8Array(total);
    for (let p = 0; p < total; p += 1) {
      const i = p * 4;
      changed[p] =
        Math.abs(a.data[i] - b.data[i]) > NOISE ||
        Math.abs(a.data[i + 1] - b.data[i + 1]) > NOISE ||
        Math.abs(a.data[i + 2] - b.data[i + 2]) > NOISE ||
        Math.abs(a.data[i + 3] - b.data[i + 3]) > NOISE
          ? 1
          : 0;
    }
...
async function main() {
  ...
  const context = await browser.newContext({
    ...
    // Запись в буфер обмена: без разрешения writeText отказывает, а
    // useCopyFeedback глотает отказ молча — «скопировано» просто не загоралось
    // бы, и снимок показывал бы кнопку в покое, ничего об этом не сказав.
